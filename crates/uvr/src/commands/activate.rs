...
pub fn run(emit: Option<ActivateShell>, write_shim: bool) -> Result<()> {
    ...
    ui::hint(
        "Run `deactivate` to restore your shell (`uvr_deactivate` when another \
         tool, such as a Python venv, already owns that name).",
    );
    Ok(())
}
...
fn emit_posix(project: &str, env: &REnv, prompt: bool) -> String {
    ...
    // idempotent rather than stacking a second R bin dir onto PATH. Calls
    // uvr's own function, never a bare `deactivate`: Python's virtualenv uses
    // that name too, and an unguarded call would tear down a venv the user
    // was working in. The UVR_PROJECT guard is kept as the cheap first test.
    out.push_str(
        "if [ -n \"${UVR_PROJECT-}\" ] && command -v uvr_deactivate >/dev/null 2>&1; \
         then uvr_deactivate; fi\n",
    );
    ...
    // uvr_deactivate: restore every saved variable, drop our bookkeeping, and
    // remove itself so the shell is left exactly as it was found. The work
    // lives under uvr's own name; `deactivate` is only an alias for it, and
    // only when no other tool owns that name (#303).
    out.push_str("uvr_deactivate() {\n");
    for name in &saved {
    ...
    // The alias goes only if this activation created it. A venv's own
    // `deactivate` must survive us.
    out.push_str(
        "    if [ -n \"${UVR_OWNS_DEACTIVATE-}\" ]; then unset -f deactivate; fi\n\
         \x20   unset UVR_OWNS_DEACTIVATE\n\
         \x20   unset -f uvr_deactivate\n",
    );
    out.push_str("}\n");

    // Take the `deactivate` name only when it is free. Defining it over a
    // Python venv's function would replace it, and `unset -f deactivate` on
    // the way out would then leave the venv with no way to deactivate at all
    // (#303). A shell has one function namespace, so the old definition
    // cannot be saved and put back.
    out.push_str(
        "if command -v deactivate >/dev/null 2>&1; then\n\
         \x20   UVR_OWNS_DEACTIVATE=''\n\
         \x20   echo \"uvr: another tool already defines 'deactivate'; \
         run 'uvr_deactivate' to leave this project.\" >&2\n\
         else\n\
         \x20   UVR_OWNS_DEACTIVATE=1\n\
         \x20   deactivate() { uvr_deactivate; }\n\
         fi\n",
    );
    out
}
...
fn emit_fish(project: &str, env: &REnv, prompt: bool) -> String {
    ...
    // uvr's own function, never a bare `deactivate`: a Python venv uses that
    // name too. See emit_posix.
    out.push_str(
        "if set -q UVR_PROJECT; and functions -q uvr_deactivate\n    uvr_deactivate\nend\n",
    );
    ...
    out.push_str("function uvr_deactivate\n");
    for name in managed_vars(env) {
        out.push_str(&format!(
    ...
    out.push_str(
        "    if test -n \"$UVR_OWNS_DEACTIVATE\"\n        functions -e deactivate\n    end\n\
         \x20   set -e UVR_OWNS_DEACTIVATE\n    functions -e uvr_deactivate\nend\n",
    );
    // Only take the `deactivate` name when it is free — see emit_posix (#303).
    out.push_str(
        "if functions -q deactivate\n\
         \x20   set -g UVR_OWNS_DEACTIVATE ''\n\
         \x20   echo \"uvr: another tool already defines 'deactivate'; \
         run 'uvr_deactivate' to leave this project.\" >&2\n\
         else\n\
         \x20   set -g UVR_OWNS_DEACTIVATE 1\n\
         \x20   function deactivate\n        uvr_deactivate\n    end\n\
         end\n",
    );
    out
}
...
fn emit_powershell(project: &str, env: &REnv, prompt: bool) -> String {
    ...
    // name too. See emit_posix.
    out.push_str(
        "if ($env:UVR_PROJECT -and (Test-Path Function:\\uvr_deactivate)) { uvr_deactivate }\n",
    );
    ...
    out.push_str("function global:uvr_deactivate {\n");
    for name in managed_vars(env) {
        out.push_str(&format!(
    ...
    out.push_str(
        "    if ($env:UVR_OWNS_DEACTIVATE) \
         { Remove-Item Function:\\deactivate -ErrorAction SilentlyContinue }\n\
         \x20   Remove-Item Env:\\UVR_OWNS_DEACTIVATE -ErrorAction SilentlyContinue\n\
         \x20   Remove-Item Function:\\uvr_deactivate -ErrorAction SilentlyContinue\n}\n",
    );
    // Only take the `deactivate` name when it is free — see emit_posix (#303).
    out.push_str(
        "if (Test-Path Function:\\deactivate) { $env:UVR_OWNS_DEACTIVATE = ''; \
         Write-Warning \"uvr: another tool already defines 'deactivate'; \
         run 'uvr_deactivate' to leave this project.\" } \
         else { $env:UVR_OWNS_DEACTIVATE = '1'; \
         function global:deactivate { uvr_deactivate } }\n",
    );
    out
}
...
mod tests {
    ...
    fn posix_defines_a_deactivate_that_removes_itself() {
        let out = emit_posix("demo", &env(), false);
        assert!(out.contains("uvr_deactivate() {"));
        assert!(out.contains("unset -f uvr_deactivate"));
        // The alias is dropped only when this activation created it (#303).
        assert!(
            out.contains(r#"if [ -n "${UVR_OWNS_DEACTIVATE-}" ]; then unset -f deactivate; fi"#)
        );
    ...
    #[test]
    fn no_shell_takes_the_deactivate_name_from_another_tool() {
        // #303: a Python venv defines `deactivate` too. A shell has one
        // function namespace, so defining ours over it replaces it, and
        // removing ours on the way out then leaves the venv with no
        // deactivate at all. Every shell therefore keeps its own
        // `uvr_deactivate` and takes the short name only when it is free.
        let env = env();
        for (shell, script, guard) in [
            (
                "posix",
            ...
            assert!(
                script.contains("uvr_deactivate"),
                "{shell}: no uvr-owned name:\n{script}"
            );
            let taken = script
                .find(guard)
                .unwrap_or_else(|| panic!("{shell}: the name is taken unconditionally:\n{script}"));
            let defines = script
                .rfind("deactivate")
                .expect("a script that defines nothing");
            assert!(
                taken < defines,
                "{shell}: the alias is defined before the name is checked:\n{script}"
            );
            assert!(
                script.contains("UVR_OWNS_DEACTIVATE"),
                "{shell}: nothing records who owns the name:\n{script}"
            );
        }
    }

    #[test]
    fn every_shell_unwinds_through_its_own_function() {
        // The unwind must never call a `deactivate` it does not own (#303).
        let env = env();
        for (shell, script) in [
            ("posix", emit_posix("demo", &env, false)),
            ("fish", emit_fish("demo", &env, false)),
            ...
            let first = script
                ...
                .unwrap_or_else(|| panic!("{shell}: no unwind at all"));
            assert!(
                script[first.saturating_sub(4)..].starts_with("uvr_deactivate"),
                "{shell}: the unwind calls a foreign deactivate:\n{}",
                &script[..first + 40]
            );
        }
    }
    ...
    fn fish_prepends_to_the_path_list_and_exports_every_var() {
        ...
        assert!(out.contains("if set -q UVR_PROJECT; and functions -q uvr_deactivate"));
    }
    ...
    fn powershell_prepends_to_path_and_exports_every_var() {
        ...
        assert!(out.contains("if ($env:UVR_PROJECT -and (Test-Path Function:\\uvr_deactivate))"));
    }
